...
type PromoTemplate = 'lightning' | 'blueprint' | 'satoshi' | 'minimal';
...
export default function PromotionHubPage() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<PromoTemplate>('lightning');
    ...
    try {
      ...
      const result = await generateSocialPromotion({
        ...
        reward: assetType === 'task' ? `${selectedAsset.reward_amount} SAT` : 
                assetType === 'project' ? `${selectedAsset.budget_min || selectedAsset.budget?.min || 0}-${selectedAsset.budget_max || selectedAsset.budget?.max || 0} SAT` : 
  ...
  const templates: { id: PromoTemplate; label: string; icon: any }[] = [
    { id: 'lightning', label: 'Cyber Lightning', icon: Zap },
    { id: 'blueprint', label: 'Tech Blueprint', icon: Layout },
    { id: 'satoshi', label: 'Satoshi Gold', icon: Cpu },
    { id: 'minimal', label: 'Modern Minimal', icon: Monitor },
  ];
          ...
          {selectedAsset && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <Card className="glass-card border-none rounded-[2rem]">
                  <CardHeader className="p-8 pb-4">
                    <CardTitle className="text-lg font-headline flex items-center gap-2">
                      <Layout className="w-5 h-5 text-primary" /> Visual Synthesis Theme
                    </CardTitle>
                  </CardHeader>
                  ...
                  <CardHeader className="p-8 pb-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div>
                      <CardTitle className="font-headline text-2xl flex items-center gap-3">
                        <Sparkles className="w-6 h-6 text-primary" /> AI Copy Generator
                      </CardTitle>
                      <CardDescription>Synthesize compelling text for social rails.</CardDescription>
                    </div>
                    <Button 
                  ...
                  <CardContent className="p-8 pt-0 space-y-6">
                    {!promoContent && !isGenerating ? (
                      <div className="py-12 text-center border-2 border-dashed border-white/5 rounded-3xl space-y-4">
                        ...
                        <p className="text-sm text-muted-foreground max-w-xs mx-auto">Generate AI copy signals to match your visual theme.</p>
                      </div>
                    ) : isGenerating ? (
                      <div className="py-20 text-center space-y-4">
                        <div className="flex justify-center"><Loader2 className="w-12 h-12 text-primary animate-spin" /></div>
                        <p className="text-xs font-bold uppercase tracking-[0.3em] text-primary animate-pulse">Synthesizing Signals...</p>
                      </div>
                    ) : (
                      <div className="space-y-8">
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                              <ChevronRight className="w-3.5 h-3.5" /> Thread Hook
                            </div>
              ...
              <div className="lg:col-span-5 space-y-6">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground ml-2">Visual Signal Node</h3>
                
                <div className="relative group perspective-1000">
                  <SocialCard asset={selectedAsset} type={assetType} template={selectedTemplate} cardRef={cardRef} />
                  ...
                  <div className="absolute inset-0 bg-black/60 backdrop-blur-md rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center gap-6 z-20">
                     <div className="text-center space-y-2 mb-4 px-8">
                       ...
                       <p className="text-xs text-white/60">Export 4K PNG for professional propagation.</p>
                     </div>
                     <div className="flex flex-col sm:flex-row gap-4 px-8 w-full justify-center">
                       ...
                       >
                         <Eye className="w-5 h-5" /> Fullscreen
                       </Button>
                     </div>
                  </div>
                </div>
                ...
                <Card className="glass-card border-none rounded-[2rem] p-8 space-y-6">
                   <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="font-bold">Propagation Hub</h4>
                      </div>
                      <Badge variant="outline" className="border-white/10 uppercase text-[8px] font-bold tracking-widest px-2">L2 Rail</Badge>
                   </div>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-5xl bg-background border-white/10 p-0 overflow-hidden rounded-[2.5rem]">
          <DialogHeader className="p-8 pb-4">
            <DialogTitle className="text-2xl font-headline font-bold">4K Signal Propagation Preview</DialogTitle>
          </DialogHeader>
          <div className="p-8 pt-0">
             <div className="scale-110 origin-top transform my-12">
               <SocialCard asset={selectedAsset} type={assetType} template={selectedTemplate} />
             </div>
...
function SocialCard({ asset, type, template, cardRef }: { asset: any, type: any, template: PromoTemplate, cardRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  const getTemplateStyles = () => {
    switch (template) {
      case 'minimal':
        return {
          container: "bg-[#0A0A0A] border-[#333] shadow-none",
          headerBg: "bg-white/5 border-white/10",
          titleText: "text-white font-medium",
          pill: "bg-white/10 border-white/20 text-white",
          accentColor: "text-white"
        };
      case 'blueprint':
        return {
          container: "bg-[#1A1A2E] border-[#2E2E4E] border-dashed",
          headerBg: "bg-[#2E2E4E]/30 border-[#2E2E4E]",
          titleText: "text-[#00D1FF] font-mono",
          pill: "bg-[#00D1FF]/10 border-[#00D1FF]/20 text-[#00D1FF]",
          accentColor: "text-[#00D1FF]"
        };
      case 'satoshi':
        return {
          container: "bg-gradient-to-br from-[#1C1917] to-[#0A0A0A] border-[#F59E0B]/30",
          headerBg: "bg-[#F59E0B]/10 border-[#F59E0B]/20",
          titleText: "text-[#F59E0B]",
          pill: "bg-[#F59E0B]/20 border-[#F59E0B]/40 text-[#F59E0B]",
          accentColor: "text-[#F59E0B]"
        };
      case 'lightning':
      default:
        return {
          container: type === 'task' ? "bg-gradient-to-br from-[#8457F1] via-[#6366F1] to-[#3C62FF] border-[#8457F1]/30" :
          ...
          headerBg: "bg-white/20 border-white/40",
          titleText: "text-white",
          pill: "bg-black/20 border-white/20 text-white",
          accentColor: "text-white"
        };
    }
  };
    ...
    <div ref={cardRef} className={cn(
      ...
      <div className="relative z-10 flex justify-between items-start">
        <div className="flex items-center gap-4">
          <div className={cn("w-16 h-16 rounded-2xl backdrop-blur-2xl flex items-center justify-center border shadow-2xl", styles.headerBg)}>
          ...
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-80">GigaLight Protocol</p>
            <Badge className={cn("border text-[8px] font-bold uppercase tracking-widest px-3", styles.pill)}>
              {type === 'task' ? 'MICRO GIG' : type === 'project' ? 'STRATEGIC PROJECT' : 'EXPERT SERVICE'}
            </Badge>
          </div>
        </div>
        <div className={cn("backdrop-blur-md px-5 py-2.5 rounded-2xl border flex items-center gap-2", styles.headerBg)}>
          <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-[10px] font-bold uppercase tracking-widest">LIVE SIGNAL</span>
        </div>
      </div>

      <div className="relative z-10 space-y-6">
        <h2 className={cn("text-4xl sm:text-5xl font-headline font-bold leading-[1.1] tracking-tight drop-shadow-2xl", styles.titleText)}>
          {asset.title}
        </h2>
        <div className="flex flex-wrap gap-2.5">
           {(asset.skills || []).slice(0, 4).map((s: any, i: number) => (
             <span key={i} className={cn("px-4 py-1.5 rounded-xl border text-[10px] font-bold uppercase tracking-widest backdrop-blur-xl", styles.pill)}>
      ...
      <div className="relative z-10 flex items-end justify-between border-t border-white/10 pt-8 mt-2">
         <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60">TECHNICAL YIELD</p>
            <p className={cn("text-4xl font-headline font-bold tracking-tighter", styles.accentColor)}>
              {type === 'task' ? `+${asset.reward_amount?.toLocaleString()} SAT` : 
               type === 'project' ? `${asset.budget_min?.toLocaleString() || asset.budget?.min?.toLocaleString() || 'TBD'} SAT` : 
               `${asset.price_sats?.toLocaleString()} SAT`}
            </p>
         </div>
         <div className={cn(
           "flex items-center gap-3 px-8 py-3.5 rounded-[1.5rem] font-bold text-sm shadow-2xl hover:scale-105 transition-transform cursor-default",
           template === 'satoshi' ? "bg-[#F59E0B] text-black" : 
           template === 'minimal' ? "bg-white text-black" :
           template === 'blueprint' ? "bg-[#00D1FF] text-black" : "bg-white text-black"
         )}>
