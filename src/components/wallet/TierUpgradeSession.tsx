...
interface TierUpgradeSessionProps {
  paymentData: {
...
export function TierUpgradeSession({ paymentData, onSuccess, onCancel }: TierUpgradeSessionProps) {
  ...
  useEffect(() => {
    ...
    pollingIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        console.warn("Activation check failed, retrying...");
      }
    }, 3000);
      ...
      <div className="space-y-8 text-center animate-in zoom-in-95 duration-500 py-10 px-4">
        ...
        <div className="space-y-2">
          <p className="text-2xl sm:text-3xl font-headline font-bold text-white">
            {paymentData.tier?.display_label || 'Pro'} Class Activated
          </p>
          <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-[0.3em]">Protocol Upgrade Confirmed</p>
        </div>
    ...
    return (
      <div className="space-y-8 text-center animate-in zoom-in-95 duration-500 py-10 px-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">The activation path has timed out. Please re-initialize.</p>
        </div>
        <Button variant="outline" className="w-full h-14 rounded-2xl border-white/10 font-bold" onClick={onCancel}>
          Return to Identity Config
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 text-center animate-in zoom-in-30 duration-300">
      <div className="mx-auto bg-white p-4 sm:p-6 rounded-[2.5rem] w-full max-w-[280px] aspect-square shadow-2xl shadow-primary/20 border-8 border-primary/10 relative overflow-hidden">
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary/20 bg-primary/10">
          <Zap className="w-3.5 h-3.5 text-primary" />
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Tier Activation</span>
        </div>
      ...
      <div className="space-y-4 px-2">
        ...
        <div className="flex items-center gap-3 bg-black/40 border border-white/10 rounded-2xl p-4 overflow-hidden text-left relative">
          ...
          <Button 
            size="icon" 
            variant="secondary" 
            className="h-10 w-10 shrink-0 rounded-xl neon-glow-primary"
            onClick={() => {
